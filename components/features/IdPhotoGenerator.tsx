...
const IdPhotoGenerator: React.FC<IdPhotoGeneratorProps> = ({ sessionToLoad, onSaveSession, imageToLoad, onSendImage }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (mode === 'single') {
        ...
        try {
          const newImage = await generateIdPhoto(baseImage, options, modelType);
        ...
        for (let i = 0; i < filesToProcess.length; i++) {
            ...
            try {
                const newImage = await generateIdPhoto(file.dataUrl, options, modelType);
  ...
  const handleRegenerateBatchItem = async (index: number) => {
    ...
    try {
        const newImage = await generateIdPhoto(itemToRegenerate.original, options, modelType);
