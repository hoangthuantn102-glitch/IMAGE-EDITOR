...
import type { IdPhotoOptions, BeautifyPhotoParams, InvitationGeneratorParams, MaketGeneratorParams } from '../types';
...
export const suggestMaketBackground = async (
  mainContent: string,
  leftTitle?: string,
  centerTitle?: string,
  rightTitle?: string
  ...
  const ai = new GoogleGenAI({ apiKey: getApiKey() });
  const contextParts = [
    `Nội dung chính maket / sự kiện: "${mainContent}"`,
    leftTitle ? `Tiêu đề trái: "${leftTitle}"` : '',
    centerTitle ? `Tiêu đề giữa: "${centerTitle}"` : '',
    rightTitle ? `Tiêu đề phải: "${rightTitle}"` : '',
  ].filter(Boolean).join('\n');

  const prompt = `Bạn là chuyên gia thiết kế mỹ thuật sân khấu, backdrop, phông bạt maket hội nghị và sự kiện chuyên nghiệp.
Dựa vào thông tin sự kiện sau:
${contextParts}

Hãy phân tích tính chất, chủ đề và không khí của sự kiện (ví dụ: hội nghị trang trọng, đại hội đảng/đoàn, tổng kết công ty, gala tiệc tối, hội thảo công nghệ, lễ hội âm nhạc, lễ kỷ niệm/tri ân, tiệc mừng, v.v.).
Hãy đưa ra 4 gợi ý mô tả phong cách nền (background) maket đẹp mắt, ấn tượng và cực kỳ phù hợp bằng tiếng Việt.
Mỗi gợi ý là 1 đoạn mô tả ngắn gọn (1-2 câu) về màu sắc nền chủ đạo, hoa văn trang trí (trống đồng, dải lụa, sóng công nghệ LED, hoa văn sang trọng...), hiệu ứng ánh sáng đèn sân khấu hoặc biểu tượng.
Trả về định dạng JSON object: { "suggestions": ["gợi ý 1", "gợi ý 2", "gợi ý 3", "gợi ý 4"] }`;
...
export const generateMaketImage = async (params: MaketGeneratorParams): Promise<string[]> => {
  const {
    leftTitle,
    centerTitle,
    rightTitle,
  ...
  const formatTitleLines = (text: string) => {
    const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
    if (lines.length > 1) {
      return `trình bày trên ${lines.length} dòng ngay ngắn:\n${lines.map((l, i) => `    + Dòng ${i + 1}: "${l}"`).join('\n')}`;
    }
    return `"${lines[0] || text.trim()}"`;
  };

  const topHeaderParts: string[] = [];
  if (leftTitle?.trim()) {
    topHeaderParts.push(`- Góc trên cùng bên trái (căn lề trái, ngay ngắn, trang trọng, có thể trình bày 1 hoặc 2 dòng): ${formatTitleLines(leftTitle)}.`);
  }
  if (centerTitle?.trim()) {
    topHeaderParts.push(`- Chính giữa trên cùng (căn giữa, trang trọng, trên đỉnh maket, có thể trình bày 1 hoặc 2 dòng): ${formatTitleLines(centerTitle)}.`);
  }
  if (rightTitle?.trim()) {
    topHeaderParts.push(`- Góc trên cùng bên phải (căn lề phải, ngay ngắn, tên đơn vị hoặc khẩu hiệu, có thể trình bày 1 hoặc 2 dòng): ${formatTitleLines(rightTitle)}.`);
  }

  const prompt = `Bạn là chuyên gia thiết kế đồ họa sân khấu, phông bạt (maket) và backdrop sự kiện đỉnh cao.
Hãy thiết kế maket sân khấu hoàn chỉnh với bố cục đồ họa chuyên nghiệp, trang trọng và chuẩn mực cao nhất:

1. HÀNG TIÊU ĐỀ TRÊN CÙNG (TOP HEADER):
${topHeaderParts.length > 0 ? topHeaderParts.join('\n') : '- Hàng tiêu đề trên cùng để thoáng, không có chữ.'}
(Lưu ý: Các tiêu đề trên cùng phải nằm sát phần đỉnh trên, phân bố đúng vị trí trái - giữa - phải).

2. NỘI DUNG CHÍNH (TRỌNG TÂM MAKET - CHÍNH GIỮA):
- Hiển thị văn bản: "${mainContent.trim()}"
- Vị trí: Đặt ở CHÍNH GIỮA maket (center), phông chữ to nhất, nổi bật nhất, kiểu chữ in hoa hoặc nghệ thuật trang trọng, có bóng đổ hoặc hiệu ứng nổi khối 3D ánh kim đẹp mắt, dễ đọc và sắc nét.

3. ĐỊA DANH, NGÀY THÁNG (GÓC DƯỚI BÊN PHẢI):
${locationDate?.trim() ? `- Hiển thị văn bản: "${locationDate.trim()}"
- Vị trí: Nằm ở khu vực góc bên dưới bên phải (bottom-right), kiểu chữ nghiêng hoặc thanh lịch.
- QUY TẮC BỐ CỤC: KHÔNG đặt sát mép đáy dưới cùng và KHÔNG đặt lệch quá sát mép cạnh phải (đảm bảo có khoảng lề an toàn để không bị che khuất khi dựng sân khấu, tạo thế cân bằng trang nhã).` : '- Không có dòng địa danh ngày tháng.'}

4. PHONG CÁCH VÀ PHÔNG NỀN (BACKGROUND THEME):
${backgroundPrompt?.trim() ? `- Phông nền được thiết kế theo mô tả: ${backgroundPrompt.trim()}` : '- Phông nền sân khấu sự kiện sang trọng, chuyên nghiệp, ánh sáng hài hòa làm nổi bật nội dung chính.'}

5. TIÊU CHUẨN THIẾT KẾ:
- Tỷ lệ khung hình: ${aspectRatio}.
- Phông chữ tiếng Việt đầy đủ dấu, chuẩn chính tả tiếng Việt.
- Bố cục hài hòa chuẩn thiết kế sân khấu sự kiện, màu sắc sắc nét, độ tương phản cao, chuyên nghiệp.`;
